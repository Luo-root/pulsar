...
| 功能 | 说明 |
|---|---|
| 配置面 | 单一 config（模型 / 工具策略 / 技能根 / 记忆 / 调度 / 审批策略 / 外部 Agent 清单）——**不含工作区根**（见下一行） |
| 工作区 | **会话 / 项目级属性，不是 runtime 属性**：建会话时由调用方给出（CLI `-C`，缺省 = 当前目录）、写进 `SessionHeader.Workspace` 随会话持久化；续跑以会话记录为准，调用方给了不一致的**直接拒绝**（不把「接着上次聊」接到另一个项目上）；工具面按该工作区**每回合装配**。配置只承载**策略**（写根 / 读禁 / 工具子集），其相对路径在回合时相对生效的工作区解析。项目作用域的记忆与技能都挂在这个工作区上（§2.5） |
...
| **WASM 插件**（`wazero`，纯 Go 无 CGO） | ✓ | **真沙箱**（只能碰宿主导入面） | 同上 | 导入面设计 + 调试成本 | **模型 / 陌生人写的代码的安全落点**——它不是「可选档位」，而是「放心让模型自由造插件」的前提（**已定：与进程档同在 v0.5**） |

**两档的区别（别搞反）**：外部进程 = **用「我信它」换语言与生态自由**——插件是宿主起的另一个进程，它跑在你自己的权限下，能干的活跟你在终端里能干的一样多；宿主的控制只在「给它什么 env / cwd / stdio 上的协议」。WASM = **用「能力受限」换「不必信它」**——插件只能调宿主导入的那几个函数，没有导入文件读写它就读不了文件，没有导入 socket 它就联不了网。**越权尝试在这档里不是「被拦住」，是根本不存在那个接口。**

典型的消费方分工：要连数据库、要调外部 API、要跑几分钟长任务、要用 Node/Python 生态 → 进程档；「从网上抄来的一段变换逻辑，或者模型现写的校验器/清理器」 → WASM 档。

**能改 UI 不是选档位的理由**：UI 半永远是一段跑在用户浏览器里的 JS（浏览器的事实），两种来源都必须附 `client.js`，而那段 JS 永远是全信任的（读得到页面数据、带得到会话凭据）。所以「插件能改 UI」与它服务端半是进程还是 WASM **无关**——真正与它相关的是：带 UI 半的插件，安装必须人批准。

**还有第三条路**（进程档的安全补丁）：OS 级沙箱（Linux landlock / Windows ACL 或 job object）可以给进程插件加限制——DSH 就是「进程沙箱 + vm」两条线都留着。它比 WASM 更贴近原语言生态，但粒度粗、平台相关，**不是能力制**，只能算缓解。

**WASM 档要一并设计的东西（v0.5 范围内，2026-09-23 决议）**：

- **导入面（第一版）**：`log` / `config_get` / `emit_event` / 插件私有 `state_get`·`state_set`，以及**按权限接上**的 `fs_read` / `fs_write` / `http_fetch`；宿主能力一律走**显式列出的桥**（如 `tool_invoke` 且照过审批门），不开放「宿主对象随便摸」。
- **「强制」的含义**：清单里的 `permissions` 在**只有 WASM 档**会真的决定哪些导入被接上；进程档与 UI 半的 `permissions` 仍然只是**声明与展示**。这条要说清楚，别让人误以为所有档位都被强制。
- **ABI 与脚手架**：导入面用 WIT 描述，配最小模板（Rust / Go→wasm）+ `pulsar plugin new` / `pulsar plugin build`。这是「模型能造插件」的**实际前提**——模型写完代码得有可复现的编译路径，否则「造」只是嘴上说说。
...
#### 2.10.7 模型与插件：分档给，不在「安装」上设假门

**结论（2026-09-23 修订）**：模型**可以**装插件、组合插件、造插件——门不该设在「安装」这个动作上，该设在**信任级别**上。

理由：一个有 bash 与文件工具的模型，本来就能往插件目录里写文件、改装配清单。假装「拦住安装」只是安全剧场。真正决定风险的不是**谁写了文件**，而是**这段代码以什么权限跑**。

先分清内核做到了什么、做不到什么：`Loader.Reconcile` 的容错（条目标识稳定、单条目失败不阻断其余、`Apply` panic 被 recover 成 `Failed`、改配置即重建、改回去即回滚）保护的是**运行时稳定性**——所以「模型改装配」在结构上是安全的。它**不保护权限**：一个装载成功的插件能做的事，跟写它的模型能做的事一样多（进程档尤其如此）。权限只有一档能给——WASM 的能力制。

于是三件事分开对待：

| 模型的动作 | 机制 | 门设在哪 | v0.5 |
|---|---|---|---|
| **组合**（改 profile / 启停条目 / 调 config） | `Loader.Reconcile`（底座已有） | 每次改动**留痕** + UI 一键回滚；不必逐次人批（结构上安全） | 给 |
| **装**（放进目录 + 清单校验 + 启用） | 清单解析 + 权限声明展示 | **一次人审批**（安装即信任），之后可反复跑 | 给 |
| **造（进程档）** | 模型写 `server.js` 之类 | 与 bash 工具**同档**——明说，不假装更安全，也不额外收紧 | 给 |
| **造（WASM 档）** | 模型写 Rust / Go → wasm，宿主只给导入面 | 无需人信这段代码 | **给（v0.5，与进程档同版）** |
| **带 UI 半的插件**（任何档位） | `client.js` 在用户浏览器里跑 | **必须人批准**——浏览器半永远是全信任代码 | 人批后给 |

> 这正是 DSH 的做法：它给「模型动态写的插件」专门开了通道（host 半跑 `node:vm`，带浏览器半的包须人工审批后才可运行），只是它自己承认那个 vm「**不是安全边界**」。pulsar 的区别是不假装 vm 是边界：把「模型自由写代码」与 **WASM 档位**绑在一起——在 WASM 到位之前，模型造出来的进程插件一律按 bash 同档对待（能跑，但没有任何特权）。

#### 2.10.8 与 `toolset` / 工作流 / 调度器的接缝
...
### 4.10 「零重配」怎么做到：任务级注入，不接管别人的配置

用户要的效果是：**在 pulsar 里直接换一个新的 Agent 干活，不用先去它那边重新配置任何东西。**

这个目标**可达**，但机制不是「pulsar 接管外部 Agent 的主配置」（那是最脆的做法：各家格式各自演进，写坏 = 破坏用户的开发环境），而是**任务级注入**——pulsar 在派活那一刻把它该有的东西注进去，用完即走，不落进别人的配置文件：

| 要注入的东西 | 注入通道 | 覆盖范围 | 状态 |
|---|---|---|---|
| **MCP 服务**（含 pulsar 自己的只读记忆 / 技能服务） | ACP `session/new` 的 `mcpServers` 参数 | **全 ACP 生态通用**，一次协议调用 | 协议已定（§4.8 ②） |
| **凭据**（模型 API key 等） | spawn 子进程时给 env（密钥由 pulsar 持有，按任务最小化下发） | 各产品普遍从环境变量读密钥 | **待 spike**：逐个核对变量名与优先级 |
| **模型选择** | ACP `session/set_config_option`（支持时）/ 适配器启动参数 | DSH 支持；其余看 `initialize` 能力位 | 能力位，不假设 |
| **工作目录与边界** | ACP `session/new` 的 `cwd` | 全 ACP 生态 | 协议已定 |
| **用户画像 / 项目约束** | 投影到 `AGENTS.md` + 派活时随任务书给 | 读 `AGENTS.md` 的产品 | §4.8 ①，待 spike |
| **技能** | Agent Skills 目录投影 / 经 MCP 读取 | 原生支持技能目录的产品 | §4.8 ①，待 spike |

于是「配置托管」的正确形状是三层，按**侵入性从低到高**：

1. **注入（默认，必做）**——不碰外部 Agent 的任何配置文件，效果已覆盖上表绝大部分。**这就是用户要的那个便捷性**。
2. **探测（只读）**——读它们的配置只为**展示与对账**（这个 agent 现在用什么模型、挂了哪些 MCP），不写。
3. **投影 / 导出（可选，带确认）**——把 pulsar 的共享面写到约定落点（`AGENTS.md`、技能目录）。边界**严格限定在 pulsar 自己的共享面**，绝不写别人的主配置；每次投影带 diff 预览与备份。

**明确不做**：由 pulsar 生成并覆盖外部 Agent 的模型 / MCP / 凭据主配置。理由不是「做不到」，而是**不划算且危险**——N 个产品 N 套格式且都在演进，写坏一次的代价是用户的开发环境被破坏，而相对注入的额外收益几乎为零。

**怎么验收这条成立**（v0.5 的 spike）：装一个 pulsar **从没见过的** ACP agent（例如 Kimi CLI 或 Gemini CLI），**不改它的任何配置**，直接派一个任务——pulsar 注入的 MCP 工具与凭据应当立刻可用。跑得通，「零重配」就是真的。
...
- **前端栈（已定）**：**React + Vite + TypeScript**，独立 SPA；构建产物由 `Engine.Static` 托管，API 纯 JSON/SSE。选它的直接理由：插件浏览器半要走原生 ESM 动态 `import()`，而 React 作为**宿主统一提供**的共享依赖，是这条路上最少摩擦的组合（§2.10.4）。
...
| v0.5 | **插件系统 v1（动态加载，双档来源）** | 清单 + 目录扫描 + **进程档与 WASM 档**（协议握手 / `wazero` + 导入面 + WIT + `pulsar plugin new·build` 脚手架）+ UI 槽位挂载 + schema 表单 + 启停与权限展示 + **模型可用的组合 / 安装 / 创作面**（§2.10.7）。这是迄今最大的一版：**同版内先打通进程档 + 装/组合，再接 WASM 档**，顺序可切、提测点分开，但版本边界不再往后挪。放这里是因为它需要宿主面先长齐（工具、命令、设置页、UI 槽都有东西可挂），而工作流的节点面板正好第一个用上它 |
...
| 之后 | 纵深 | 兜底 `runner/exec` 适配器、从会话沉淀工作流、pulsar 自身作为 ACP server（让 Zed/JetBrains 驱动 pulsar）、A2A 观望 |
...
| 12 | **「零重配」由任务级注入实现，不接管外部 Agent 的主配置**：注入（默认）→ 只读探测 → 可选投影（带 diff 与备份）；「由 pulsar 覆盖别人主配置」明确不做 | 2026-09-23 | §4.10 |
| 13 | SPA 技术栈 = **React + Vite + TypeScript** | 2026-09-23 | §6.1 |
| 14 | 插件来源 v1 = **外部进程插件 + UI 槽位**；WASM 档位后置（2026-09-23 修订：§2.10.7 已明确「模型自由造插件」以 WASM 为前提，落点时机见 §8.2） | 2026-09-23 | §2.10.2 / §2.10.7 / §7 |
| 15 | v0.1 的完成定义 = §8.3 的端到端场景（不是功能面清单） | 2026-09-23 | §7 / §8.3 |
| 16 | 模型**可以**装 / 组合 / 造插件；门设在**信任级别**上——组合留痕可回滚、安装一次人批、进程档造插件按 bash 同档、带 UI 半必须人批 | 2026-09-23 | §2.10.7 |
| 17 | **WASM 档位提前进 v0.5**（与进程档同版）：导入面 + WIT + 最小模板与 `pulsar plugin new/build` 脚手架一起做——让「模型自由造插件、宿主不必信」从 v0.5 起就成立 | 2026-09-23 | §2.10.2 / §7 |
| 18 | **工作区是会话 / 项目级属性**（不是 runtime 属性）：随会话头持久化、续跑以记录为准、工具面每会话装配；配置只放策略，工作区根不落配置文件 | 2026-09-23 | §2.9 / §6.2 |

### 8.2 仍待拍板

**没有未决项了**（2026-09-23；原第 A 项「WASM 档位放哪」已按决议 17 拍定进 v0.5）。下一个决策点是 v0.2 开工前的**前端脚手架评审**（路由 / 状态管理 / 组件库选型，以及 Slot 注册表在 React 里的具体形状）——它不阻塞 v0.1。

要盯的风险（不是待拍板，是待验证）：§4.10 的「凭据注入」与 §4.8 ① 的「投影矩阵」都需要**真机 spike**，各产品的落点是查出来加试出来的，不能靠推断。
