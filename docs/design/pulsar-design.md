...
| **WASM 插件**（`wazero`，纯 Go 无 CGO） | ✓ | **真沙箱**（只能碰宿主导入面） | 同上 | 导入面设计 + 调试成本 | 「敢装别人写的插件」时的档位——**v1 不做**，后置到纵深（决议 14） |
...
| 14 | 插件来源 v1 = **外部进程插件 + UI 槽位**；WASM 档位后置到纵深 | 2026-09-23 | §2.10.2 / §7 |
| 15 | v0.1 的完成定义 = §8.3 的端到端场景（不是功能面清单） | 2026-09-23 | §7 / §8.3 |

### 8.2 仍待拍板

**没有未决项了**（2026-09-23）。下一个决策点是 v0.2 开工前的**前端脚手架评审**（路由 / 状态管理 / 组件库选型，以及 Slot 注册表在 React 里的具体形状）——它不阻塞 v0.1。
