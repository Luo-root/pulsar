...
| **WASM 插件**（`wazero`，纯 Go 无 CGO） | ✓ | **真沙箱**（只能碰宿主导入面） | 同上 | 导入面设计 + 调试成本 | **模型 / 陌生人写的代码的安全落点**——它不是「可选档位」，而是「放心让模型自由造插件」的**前提**（时机见 §8.2） |
...
| **造（WASM 档）** | 模型写 Rust / Go → wasm，宿主只给导入面 | 无需人信这段代码 | 档位时机待定（§8.2） |
...
| # | 问题 | 影响 | 我的建议 |
|---|---|---|---|
| A | **WASM 档位放哪**（§2.10.7 修订带出来的新问题） | 决定「模型自由造插件」何时真的安全可用 | **后置到 v0.6**：v0.5 先给「装 + 组合 + 一次人批」与进程档造插件（按 bash 同档明说），WASM 作为独立档位补上——跨度小、先能用 |

其余无未决项（2026-09-23）。下一个决策点是 v0.2 开工前的**前端脚手架评审**（路由 / 状态管理 / 组件库选型，以及 Slot 注册表在 React 里的具体形状）——它不阻塞 v0.1。
